# Requirements Archive: v1.0 UX Polish & Scheduler Improvements
...
- [x] **SCHED-01**: Scheduling engine applies a configurable penalty when a player has played a singles (1v1) match this session
- [x] **SCHED-02**: Scheduling engine applies a configurable penalty when a player was the solo side of a 3-way (2v1) match this session
- [x] **SCHED-03**: Scheduling engine applies a configurable penalty when a player was on the pair side of a 3-way (2v1) match this session
- [x] **SCHED-04**: Penalty values for SCHED-01–03 are configurable via sliders in the Settings screen with sensible defaults
- [x] **SCHED-05**: Sessions created before deploy fall back to default penalty values (no breaking schema change)

### Test Coverage
- [x] **TEST-01**: Tests verify played rounds retain their courts and players when a new player is added mid-session
- [x] **TEST-02**: Tests verify played rounds retain their courts and players when a player is removed mid-session (including players who appeared in played courts)
- [x] **TEST-03**: Tests verify that only unplayed rounds are affected by roster changes
